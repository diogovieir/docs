...
```python
from RagPUREAI_extract import DOCXLoader
...
Use this method to count the occurrences of a specific word in a DOCX document:

```python
docx_loader.GetKeywordOccurrences("/path/to/file.docx", "keyword")
# docx_loader.GetKeywordOccurrences("/path/to/directory", "keyword")
```
