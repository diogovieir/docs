...
```python
from RagPUREAI_extract import PDFLoader
...
Use this method to count the occurrences of a specific word in a PDF document:

```python
pdf_loader.GetKeywordOccurrences("/path/to/file.pdf", "keyword")
# pdf_loader.GetKeywordOccurrences("/path/to/directory", "keyword")
```
