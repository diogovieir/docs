...
```python
from RagPUREAI_extract import TXTLoader
...
To begin, create an instance of **DataExtractRequestStruct**.
...
```python
from RagPUREAI_extract import DataExtractRequestStruct
...
```python
txt_loader.KeywordExists("/path/to/file.txt", "keyword")
# txt_loader.KeywordExists("/path/to/directory", "keyword")
```

### GetKeywordOccurrences

Use this method to count the occurrences of a specific word in a text document:

```python
txt_loader.GetKeywordOccurrences("/path/to/file.txt", "keyword")
# txt_loader.GetKeywordOccurrences("/path/to/directoy", "keyword")
```
